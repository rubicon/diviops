...
| `diviops_content_search` | plugin | `search`, `post_types?`, `limit?`, `max_matches_per_post?`, `context_chars?` | true | Search every page/post on the site for a literal string and report where it occurs. … |
...
| `diviops_module_lock` | plugin | `page_id`, `label?`, `match_text?`, `auto_index?`, `occurrence?`, `dry_run?`, `backup?` | true | Lock a module so VB users cannot edit it. … |
...
| `diviops_module_unlock` | plugin | `page_id`, `label?`, `match_text?`, `auto_index?`, `occurrence?`, `dry_run?`, `backup?` | true | Unlock a module by removing attrs.locked entirely. … |
