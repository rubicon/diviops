...
registerPluginTool(
  ...
  {
    description:
      "Restore one guarded rollback snapshot to its original post/page or Theme Builder layout target. Requires target edit permission, refuses content or captured Divi post-meta checksum drift with conflict before mutation, has no force override, writes through the shared full-content integrity/readback guard, restores captured supported post meta, invalidates Divi cache, and marks restore_applied only after verified readback. dry_run previews without writing. This MVP does not create a second pre-restore snapshot. " +
      "RUN SNAPSHOTS (#199): `diviops_preset_reassign` records one run record per 100 pages instead of one per page, so a large apply no longer evicts its own snapshots. A restore addresses ONE chunk, not a whole run — a run over 100 pages is stored as several chunks, so reverting all of it means one call per id in `summary.rollback.chunks[]`. Omitting `page_ids` restores every page in THAT CHUNK; pass `page_ids` to restore only some of them. Pass a chunk id (`summary.rollback.chunks[]`, or any `summary.details[].snapshot_id`) — the bare `summary.rollback.run_id` is not addressable and answers 404. Each page is gated and refused independently, so one page that changed after the run does NOT block the others: a page edited since the run is refused with reason `conflict`, a page captured whose write was never recorded is refused `unrestorable`, and a page you already restored from this run is refused `already_restored` (not `conflict` — the change was this tool's own). " +
      "A run restore that refused anything is NOT ok:true — it returns ok:false with `rollback_snapshot.run_restore_partial` (HTTP 207) when some pages restored, or `rollback_snapshot.run_restore_failed` (HTTP 409) when none did, carrying `error.data.restored[]` and `error.data.refused[]` (each refusal names the page id, a `reason`, and for write failures the integrity guard's `diagnostics`). Naming a page THIS CHUNK does not cover is `invalid_input` (400) with `error.data = { snapshot_id, run_id, unknown_page_ids, covered_page_ids }`, never a silent no-op — a page in a sibling chunk of the same run is refused here too, so check `chunks[]`. An empty `page_ids` array is refused rather than treated as everything. Reading a run with `diviops_rollback_snapshot_get` returns only the pages you may edit; deleting one requires edit rights on every page it covers. Returns the standardized envelope.",
    inputSchema: {
      snapshot_id: z.string().min(1).describe("Snapshot id to restore. For a run this is a CHUNK id — `summary.rollback.chunks[]` or a `summary.details[].snapshot_id`, shaped `run_<stamp>_<hex>_c<n>`. The bare `run_id` is not addressable."),
      page_ids: z
        .array(z.number().int().positive())
        .min(1)
        .optional()
        .describe(
          "Run chunks only (ignored on a single-page snapshot): restore just these pages. Omit to restore every page in the addressed chunk. Ids the chunk does not cover are refused, not ignored — including pages that belong to a sibling chunk of the same run.",
        ),
      dry_run: z.boolean().optional().default(false).describe("Preview the checksum-bound restore without mutation."),
    },
    _meta: { idempotent: "false" },
  },
  async ({ snapshot_id, page_ids, dry_run }) => {
    const result = await wp.requestEnveloped(
      `/rollback-snapshot/restore/${encodeURIComponent(snapshot_id)}`,
      { method: "POST", body: page_ids === undefined ? { dry_run } : { dry_run, page_ids } },
    );
    return {
      content: [
...
registerPluginTool(
  ...
  {
    description:
      'Reassign a preset UUID across page content. Covers both module-level refs (`attrs.modulePreset[...]`) and attribute-level group-preset refs (`attrs.groupPreset.<slot>.presetId`), plus — for group presets — registry chain refs: module-bucket presets via top-level `groupPresets.<slot>.presetId`, group-bucket presets via `attrs.groupPreset.<slot>.presetId`. The `scope` param controls which ref types are walked (default "both", auto-selects based on new_uuid\'s bucket). Cross-bucket swaps (module ↔ group) are rejected with code \'preset.bucket_mismatch\' (HTTP 400) carrying `error.data = { old_bucket, new_bucket }`. Explicit scope mismatch with new_uuid\'s bucket returns code \'preset.scope_mismatch\' (HTTP 400) with `error.data = { scope, new_bucket }`. When `strip_inline=true` (default), strips inline attrs that duplicate the new preset\'s attrs (otherwise inline wins over preset): for module scope, strips from block root; for group scope, strips per-slot using Divi\'s own slot→target-path resolver (handles composite button groups, `-id-classes` suffix, FormField/checkbox/radio `attrName` mappings, cross-module translation). Both scopes enforce a singular-stack guard (skip strip when slot holds multiple presets). Unmappable group slots skip strip and emit a per-slot advisory at `summary.strip_advisory_per_slot[<module>::<slot>]`; neighbor slots are unaffected. Defaults to dry-run — set mode="apply" to actually rewrite. Use this to consolidate repeated inline styling into a reusable preset after creating one with diviops_preset_create. Dry-run is the only mode that scans the whole site; `mode="apply"` REQUIRES page_ids and returns code \'preset.apply_requires_page_ids\' (HTTP 400) without them, so an apply writes exactly the pages the plan reported. Every applied page is written through the readback/revert integrity guard behind a mandatory rollback snapshot. As of #199 an apply records ONE run snapshot per 100 pages rather than one per page — a per-page snapshot overflowed the 500-row retention cap, so a run over 500 pages used to evict its own earliest snapshots and still report success for pages it could no longer restore. `summary.rollback` carries `{ run_id, chunks[], pages_captured, storage_failed }`, and `summary.details[].snapshot_id` is the id of the run chunk holding that page — pass it to diviops_rollback_snapshot_restore, with `page_ids` to revert only some pages. A page is refused, and the rest of the batch continues, when its parse/serialize round trip is not byte-identical (\'preset.round_trip_lossy\'), when the write would detach a divi/global-layout wrapper (\'preset.global_layout_drift\'), or when the current user cannot edit it (\'preset.page_not_editable\'). One refusal is NOT per-page: if a rollback chunk cannot be stored, the run STOPS rather than write pages whose recovery data does not exist — the page that triggered it and every page after it are reported under \'preset.rollback_storage_failed\'. A partially failed apply is NOT a success: it returns ok:false with code \'preset.reassign_partial_failure\' (HTTP 207) carrying the complete summary, including the snapshot ids of the pages that were written, in `error.data`. Returns the standardized envelope { ok, data?, error: { code, message, hint? } }; missing/invalid inputs return code \'invalid_input\' with structured `error.data` documenting the failed field; new_uuid not in registry returns code \'not_found\'; oversized page_ids batch returns code \'preset.too_many_pages\' with `error.data = { received, max_pages }`.',
    inputSchema: {
      old_uuid: z
        .string()
      ...
      page_ids: z
        ...
        .optional()
        .describe(
          'Restrict to specific post IDs. Omit to scan all pages and posts — dry-run only. REQUIRED when mode="apply": an apply writes exactly the pages you name, so run the dry-run first and apply with the page ids it reported.',
        ),
      mode: z
        .enum(["dry-run", "apply"])
        .optional()
        .default("dry-run")
        .describe(
          '"dry-run" (default) returns the diff without writing. "apply" rewrites page content (and registry chains for group-scope swaps), and requires page_ids.',
        ),
      strip_inline: z
        .boolean()
        .optional()
