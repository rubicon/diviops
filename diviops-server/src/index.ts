...
registerPluginTool(
  "diviops_rollback_snapshot_restore",
  {
    description:
      ...
      "RUN SNAPSHOTS (#199): ids beginning `run_` cover many pages at once — `diviops_preset_reassign` records one run record per 100 pages instead of one per page, so a large apply no longer evicts its own snapshots. Pass `page_ids` to restore only some of the pages a run covers; omit it to restore all of them. Each page is gated and refused independently, so one page that changed after the run does NOT block the others: a page edited since the run is refused with reason `conflict`, a page captured whose write was never recorded is refused `unrestorable`, and a page you already restored from this run is refused `already_restored` (not `conflict` — the change was this tool's own). " +
      "A run restore that refused anything is NOT ok:true — it returns ok:false with `rollback_snapshot.run_restore_partial` (HTTP 207) when some pages restored, or `rollback_snapshot.run_restore_failed` (HTTP 409) when none did, carrying `error.data.restored[]` and `error.data.refused[]` (each refusal names the page id, a `reason`, and for write failures the integrity guard's `diagnostics`). Naming a page the run does not cover is `invalid_input` (400) with `error.data = { unknown_page_ids, covered_page_ids }`, never a silent no-op. Reading a run with `diviops_rollback_snapshot_get` returns only the pages you may edit; deleting one requires edit rights on every page it covers. Returns the standardized envelope.",
    inputSchema: {
      snapshot_id: z.string().min(1).describe("Snapshot id to restore. Ids beginning `run_` are run snapshots covering many pages."),
      page_ids: z
        .array(z.number().int().positive())
        .optional()
        .describe(
          "Run snapshots only: restore just these pages. Omit to restore every page the run covers. Ids not covered by the run are refused, not ignored.",
        ),
