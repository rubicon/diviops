...
The suite exposes **120 always-on tools** across the categories below, plus a further 30 conditionally-registered Pro tools that only appear on sites with the Pro plugin and a supported target plugin active (see [Free vs Pro](#free-vs-pro)). Per-tool descriptions, request shapes, and response payloads live in the server [README](diviops-server/README.md).
