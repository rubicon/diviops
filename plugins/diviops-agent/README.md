...
The plugin advertises 127 capability keys through the handshake. The MCP server gates its 106 plugin-backed tools against those keys and adds 12 server-local tools of its own, for **118 always-on tools** in total. Keys outnumber plugin-backed tools because some advertise a sub-feature of a tool rather than the tool itself (`variable_create_gradient`, the `*_backup` rollback keys, the `*_storage_multipath_v1` contract keys). For one row per tool, see the [per-tool reference](../../diviops-server/README.md#per-tool-reference).
