...
- `live.divi_version` and `live.divi_stale` — the same comparison for **Divi**, reported separately on purpose. `live.stale` answers "restart the MCP client"; a Divi change usually does not need that, because the capability handshake is against the plugin. What it needs is re-reading Divi's source before trusting anything derived from it — module attribute shapes, render behaviour, a line number you cited. `live.divi_warning` says which case you are in, including the one blind spot: only the version string is compared, so a hand-patched Divi at an unchanged version is not detectable. Divi on a live host has moved three times in a week, and its changes are what break us — 5.12.1 refactored the button padding gate out from under a value we write.
