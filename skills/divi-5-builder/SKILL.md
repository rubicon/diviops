...
10. **Cache invalidation**: Page, Theme Builder, canvas, revision and rollback writes invalidate the compiled CSS **for the post they changed**. Global colour, font and variable writes invalidate it **site-wide**, because a design token belongs to no single post. Each of those returns a `cache` block reporting `status` (`invalidated` or `unavailable`) and how much it freed — read it rather than assuming, since a host where WordPress cannot initialise its filesystem API reports `unavailable` and clears nothing. Preset writes invalidate **site-wide** too ([#403](https://github.com/rubicon/diviops/issues/403)) — a preset is shared across posts by definition, so changing its definition restyles every module bound to it. That covers `diviops_preset_create`, `_update`, `_delete`, `_set_default` and `_cleanup`; `diviops_preset_reassign` sweeps site-wide as well when it rewrites group chains, on top of the per-post invalidation for each page it edits. A run that writes nothing — any `dry_run`, or a `_cleanup` with nothing to clean — reports `cache: null` and deliberately sweeps nothing, so a no-op costs nothing and stays distinguishable from a change. Stale styles after any of these is a server-side cache, not a browser one — a hard refresh cannot fix it.
...
- **Button styling lives at sibling-level paths**: `button.decoration.{font, background, border, boxShadow}` for visual styling. Padding is **scope-dependent**: `module.decoration.spacing.padding` for inline buttons and `divi/button` module presets, `button.decoration.spacing.padding` for `divi/button` group presets (spacing assembled at `ButtonModule.php:775-786` via `array_replace_recursive`). **No `enable` flag is required** — VB-verified (Divi 5.4.0, page 1332): inline-styled buttons render correctly with no `button.decoration.button.desktop.value.enable` key. Render-relevant keys at `button.decoration.button.desktop.value.*` are limited to: `enable` (migration trigger — `"off"` causes Divi to strip `button.decoration`; never write without intent), `icon.*` (visible icon config — set `icon.enable: "off"` to suppress the default hover arrow), `padding` (icon-spacing gate, **not** visible-padding emitter — does not emit visible padding CSS, but its `left`/`right` keys suppress the hover-gate at `StyleDeclarations.php:152-167`; needed only when **nothing** in the button's preset stack supplies horizontal padding — Divi's default `divi/button` module preset does, so the gate is silent on a normally configured site (live-verified on Divi 5.13); when it is needed, set both `left` and `right`, since a single corner, `top` especially, is a no-op, see [presets.md → Hover-padding gate on Button group presets](references/presets.md#hover-padding-gate-on-button-group-presets--broad-scope-upstream-tracked)), and `alignment` (deprecated input forwarded to `decoration.sizing.alignment`). Anything outside this set (e.g. `backgroundColor`, `textColor`, `font`) has no consumer here — parses, validates, saves, then no-ops at render. Validators: `button_no_render_consumer` (warn — flags unrecognized keys at this depth), `button_missing_icon_enable` (warn — fires when sibling-level styling exists without explicit icon suppression).
...
- **Critical CSS selector scope**: generated `_tb_header` / `_tb_footer` selectors are layout-local, not globally safe. Any `wp_head` or child-theme CSS scaffold targeting `.et_pb_section_*_tb_header`, `.et_pb_row_*_tb_header`, etc. must be gated by the active Theme Builder layout ID or target a stable wrapper/class inside the intended layout.
...
- Button group-preset hover-padding workaround remains tracked in the roadmap. Re-measured against Divi 5.13 on 2026-09-13: the gate no longer emits a shorthand and no longer reads a single corner, so the bypass must now set both `left` and `right`. Still outstanding is a live group-preset render on an unpatched clean target, which no current site provides.
