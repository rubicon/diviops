...
All button presets store visual styling at the **sibling-level** paths (`button.decoration.{font, background, border, boxShadow}`) — VB-verified Divi 5.4.0. **Padding must go on `module.decoration.spacing.padding` — including for `divi/button` group presets.** The sibling-level `button.decoration.spacing.padding` path looks like the right home for a group preset and **emits nothing at all on Divi 5.13** (#445, measured 2026-09-13 on staging). Cause: `ButtonModule.php:775-786` builds `$button_affecting_attrs['spacing']` for the preset pass from **module-scope** sources only — `array_replace_recursive( $preset_printed_module_spacing, $module_element_attrs['spacing'] ?? [] )`, both reading `['module']['decoration']['spacing']`. Three sibling modules read the button-scope spacing in the equivalent spot instead (`WooCommerceProductAddToCartModule.php:891`, `WooCommerceCartTotalsModule.php:776`, `SocialMediaFollowItemModule.php:654`), so this is an inconsistency inside Divi rather than a rule. **This is a Divi defect we cannot patch — write module scope and it works.** Measured on a live render: a group preset storing `0.95em`/`1.4em` at the button-scope path painted no padding rule whatsoever, while its background, colour and border-radius emitted normally, so it is specifically spacing that is dropped. The fork's own `preset-cli` already writes `module.decoration.spacing.*` (`spacing-emitter.ts`) and never emits button-scope spacing, so tooling-authored presets are unaffected; this matters when hand-authoring or when reading a preset someone else wrote. No `enable: "on"` flag is required at `button.decoration.button.desktop.value` for custom styling to render. Render-relevant keys at that deep path are limited to `enable` (whose `"off"` value triggers a destructive *migration* — never write it without intent), `icon.*` (visible icon configuration), `padding` (icon-spacing gate, **not** a visible-padding emitter), and `alignment` (deprecated input forwarded to `decoration.sizing.alignment`). The `icon.enable: "off"` setting is what suppresses the default hover arrow on these presets.

### Hover-padding gate on Button group presets — broad scope, upstream-tracked

> **Re-measured 2026-09-13 against Divi 5.13 — the Divi 5.4.0 mechanism this section used to describe is gone.** Earlier revisions said the gate emitted a `padding: 0.3em 1em !important` shorthand whenever `'off' === $enable && ! $has_desktop_padding`, and that it misfired for every `divi/button` group preset "regardless of icon state". Neither still holds: `$has_desktop_padding` and the `0.3em 1em` shorthand appear nowhere in the file, and the icon-strip that used to pull icon-on presets into the gate is now unreachable. What remains is real but narrower, and the old single-corner workaround no longer works. Re-test on every Divi theme bump.

**What the gate does.** `Packages/Module/Options/Button/Style/StyleDeclarations.php:123-170` (`spacing_icon_hover_style_declaration`) declares `padding`, `padding-left` and `padding-right` `!important` at `:127-131`. When the button's `icon.enable` resolves to `"off"`, the branch at `:152-167` reads `$desktop_padding = $args['attr']['desktop']['value']['padding'] ?? $args['defaultAttrValue']['padding'] ?? []` (`:153`), derives `$effective_right_padding` / `$effective_left_padding` (`:155-156`) by falling back from the hover-state value to that desktop value, then adds `padding-right: 1em` (`:160-162`) and `padding-left: 1em` (`:164-166`) **independently, each only when its own side is missing from both**. The comment at `:158-159` states the intent: "only normalize missing side paddings. Avoid shorthand fallback so hover-top/bottom custom values are preserved." So top and bottom hover padding are safe by design; only the horizontal sides snap to `1em`, and they do so `!important` against the group-preset selector, outranking the longhand padding the preset paints.

**Icon-on presets are no longer pulled in.** The old explanation routed them into the gate via the `attrsFilter` that calls `ModuleUtils::remove_button_icon_attr_value` to strip the `icon` key, letting `$enable` fall through to a synthesized `"off"` default. That filter is guarded by `'presetGroup' === $style_group` (`ButtonModule.php:979`), and across the whole Divi 5.13 server tree `styleGroup` is only ever assigned `'preset'` (three call sites) or `'module'` (one) — never `'presetGroup'`. The same unreachable comparison sits in `PaymentButtonModule.php:951`, `WooCommerceCartTotalsModule.php:890` and `WooCommerceProductAddToCartModule.php:1097`. The icon key is therefore never stripped, and the gate fires only on presets that set `icon.enable: "off"` themselves.

**What actually triggers it.** Not a missing bypass corner — the gate fires only when **nothing in the button's resolved preset stack supplies horizontal padding**. `ButtonStyle.php:357-361` merges `$affecting_attrs['spacing']` into the `attr` the declarator reads, and `ButtonModule.php:775-786` fills that from **module-scope** spacing. So any `module.decoration.spacing.padding` carrying `left`/`right` — including the one Divi's own default `divi/button` module preset ships — satisfies the gate and it stays silent. On a normally configured site the gate never fires at all.

**The bypass, when you do need it.** Set **both `left` and `right`** on the group preset's `button.decoration.button.desktop.value.padding`. The values never paint — nothing at this path emits visible CSS — but each side is tested independently, so a one-sided bypass leaves the other side clobbered.

Measured on Divi 5.13 by invoking `spacing_icon_hover_style_declaration()` directly with the attr shapes Divi builds:

| deep-path `padding` reaching the gate | emitted |
|---|---|
| absent | `padding-right: 1em !important; padding-left: 1em !important;` |
| `{top}` only | `padding-right: 1em !important; padding-left: 1em !important;` |
| `{right}` only | `padding-left: 1em !important;` |
| `{left, right}` | (nothing) |
| `icon.enable: "on"`, default placement | (nothing) |

The single-corner bypass that worked on Divi 5.4.0 is gone. A `top`-only corner is a no-op.

Confirmed end to end on 2026-09-13 with a live frontend render on staging (scratch page, seven `divi/button` instances bound to the same `icon.enable: "off"` group preset, reading the compiled CSS out of `wp-content/et-cache/`):

| module preset in the stack | deep-path bypass | gate |
|---|---|---|
| Divi default (`1.6rem` horizontal padding) | none | silent |
| Divi default | `{top}` | silent |
| Divi default | `{left, right}` | silent |
| Divi default, group preset with `icon.enable: "on"` | none | silent |
| a preset carrying **no** padding | none | **fires** — `padding-left: 1em; padding-right: 1em` |
| a preset carrying no padding | `{top}` | **fires** — the corner is a no-op |
| a preset carrying no padding | `{left, right}` | silent |

The first four rows are the normal case: the module-scope padding already satisfies the gate, so the bypass buys nothing. The last three are the real failure mode and its fix.

**Check before working around it.** Confirm the gate actually fires on the site in front of you before spending a preset edit on it: load a page carrying the button, and grep the compiled CSS under `wp-content/et-cache/{post_id}/` for `padding-left: 1em` / `padding-right: 1em` against the button's order class. No match means the stack already supplies horizontal padding and there is nothing to fix. If it does fire, the cheapest durable fix is to give the `divi/button` **module** preset in use real `module.decoration.spacing.padding` — that is where the button's padding belongs anyway, and it closes the gate as a side effect.

**Workable patterns** (any one avoids the gate, listed in order of preference):

0. **Give the module preset real horizontal padding** *(preferred — fixes the cause)* — set `module.decoration.spacing.padding` with `left` and `right` on the `divi/button` module preset the site actually uses. Divi's own default button module preset already does this, which is why the gate is silent on most sites. If you would rather bypass without painting, set both horizontal sides at `button.decoration.button.desktop.value.padding: {"left": "0px", "right": "0px"}` instead — a single corner is not enough, and `top` alone does nothing (live-verified above). Does not affect buttons that explicitly bind a custom `groupPreset.button` (those use their own preset — see pattern 2).
...
2. **Padding inside the button group preset itself** (preferred for consolidation, **two-attr recipe**) — write padding on the `divi/button` group preset rather than chaining a separate `divi/spacing` preset. **Two attrs, two distinct roles** (VB-verified Divi 5.4.0):
    - **Visible-emit:** `button.decoration.spacing.desktop.value.padding` — historically read by the `spacing` styleProp pass and emitted against `_wrapper preset…, _wrapper preset…:hover`. **On Divi 5.13 this did not paint in a live render:** a button bound to a group preset carrying `button.decoration.spacing.padding` of `0.95em`/`1.4em` produced no padding rule at all once the module preset supplied none. The spacing assembly at `ButtonModule.php:775-786` draws from **module-scope** spacing for the preset pass, while sibling modules such as `WooCommerceProductAddToCartModule.php:891` draw from button-scope. Until that is resolved, put the button's visible padding on `module.decoration.spacing.padding`. Tracked separately — see the note under pattern 0.
    - **Gate-bypass:** `button.decoration.button.desktop.value.padding` — the `left` and `right` keys are read at `StyleDeclarations.php:153-156`; supplying both suppresses the `1em !important` side-padding the gate would otherwise add at `:160-166`. Values here never emit visible CSS — they only satisfy the gate. The four-corner example below already does this correctly.

   **Write both, and do not rely on this pattern alone on Divi 5.13.** The live check above found the visible-emit half silent: a group preset carrying `button.decoration.spacing.padding` painted nothing once the module preset supplied no padding. Writing both paths is still the right shape for the preset, and it is what the gate-bypass needs, but put the padding the button actually renders on `module.decoration.spacing.padding` until the emit path is resolved. Write the same per-corner values to both paths so the gate-bypass and the emitter agree:
...
```
   ...
   The gate's `!important` rule (and the visible-emit rule) live in the per-post compiled CSS at `wp-content/et-cache/{post_id}/`, not in the preset JSON — so the change only shows once that compiled CSS is regenerated. Since [#403](https://github.com/rubicon/diviops/issues/403) the preset write sweeps it site-wide itself and a manual `diviops_meta_flush_cache` is no longer needed; check the write's `cache` block reports `invalidated` rather than `unavailable`, and flush by hand only if it does not. (Note: Divi's VB does not currently draw a Spacing sub-panel under "Design → Button" for the deep-path attr even though the Composable Settings registry flags it, so the gate-bypass slot is reachable only via MCP. The visible-emit slot at `button.decoration.spacing.padding` is reachable from VB's standard Spacing panel under the button group.)
...
4. `diviops_preset_reassign` with `mode: "apply"` **and the `page_ids` from step 3** (apply refuses without them, so it can never touch a page the plan never showed) — actually rewrite the pages; content goes through `parse_blocks` + `serialize_blocks` (no regex surgery) so only `modulePreset` arrays and redundant inline attrs are touched. Each page is gated on its round trip being byte-identical, captured into the run snapshot, then written through the readback/revert guard; a run where any page failed returns `ok: false` (`preset.reassign_partial_failure`) with the chunk ids of the pages that were written in `error.data`
5. **To undo it**, use `summary.rollback` from step 4. Since #199 an apply records **one run snapshot per 100 pages**, not one per page — a per-page snapshot overflowed the 500-row retention cap, so a run over 500 pages evicted its own earliest snapshots and still reported success for pages it could no longer restore. `summary.rollback` gives you `{ run_id, chunks[], pages_captured, storage_failed }`, and each `summary.details[].snapshot_id` is the chunk holding that page. Pass a chunk id to `diviops_rollback_snapshot_restore` to revert every page in **that chunk**, or add `page_ids` to revert just the ones that came out wrong. A run over 100 pages is several chunks, so reverting the whole thing is one call per id in `chunks[]` — and naming a page that lives in a sibling chunk is refused, so match pages to chunks via `summary.details[].snapshot_id` — which is the usual case, since a bad reassign normally goes wrong on a subset. A page edited since the run is refused on its own (`conflict`) and its siblings still restore; a page you already reverted is refused `already_restored` rather than `conflict`, so you can tell your own restore apart from someone else's edit. A restore that refused anything returns `ok: false` (`rollback_snapshot.run_restore_partial`, or `..._failed` if nothing restored) with `restored[]` / `refused[]` in `error.data`
