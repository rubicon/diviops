...
2. **Padding inside the button group preset itself** (preferred for consolidation, **two-attr recipe**) — write padding on the `divi/button` group preset rather than chaining a separate `divi/spacing` preset. **Two attrs, two distinct roles** (VB-verified Divi 5.4.0):
    - **Visible-emit:** `button.decoration.spacing.desktop.value.padding` — the `spacing` styleProp pass at `ButtonModule.php:792-808` reads this in `presetGroup` mode (merged into module spacing via `array_replace_recursive` at lines 766-777) and emits the actual `padding: …` declarations against `_wrapper preset…, _wrapper preset…:hover`. This is what paints the button.
