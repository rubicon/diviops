...
| ---- | ------------- | ----- |
| `diviops-server/package.json` | Adds a `test:server-security` npm script (`tsc -p tsconfig.test.json && env -u DIVIOPS_WP_CLI_ALLOW node --test dist-test/__tests__/*.test.js`) that runs the new WP-CLI allowlist / filesystem path validator suite independently of the existing `build`/`test` scripts, so it isn't blocked by the unrelated `src/index.ts` build failure (#27). `regen:skill` (`node scripts/regen-module-formats.mjs`) was added by commit `1bcbea3`, the same commit that first wrote `module-formats.md`'s header claiming the tooling existed — #115's investigation found it never had been, and this change (#116) is what makes the script-target reference true. Adds `test:regen-skill` (`node --test scripts/regen-module-formats.test.mjs`) for the same reason `test:server-security` is separate: plain ESM, no `tsc` involved, so it isn't blocked by the `src/index.ts` build failure either (#116) | #27, #116 |
